...
DetectorConstruction::DetectorConstruction()
    : air(0), argongas(0), scintillator(0), C3H6(0), C2H4(0), CO2(0), ArCO2(0), CsI(0), aluminium(0), copper(0),
      lead(0), detSide1(160.0 * cm), detSide2(90.0 * cm), detSide3(160.0 * cm), detSide4(60.0 * cm),
      detHeight(5.0 * cm), deltaDet(3.0 * cm), deltaStacks(45.0 * cm), objLen(10.0 * cm), objWid(10.0 * cm),
      objHt(10.0cm), numDet(6), numChanPerDet(32), worldVisAtt(0), hodoscopeVisAtt(0), chamberVisAtt(0)
{
}
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    G4LogicalVolume* gemPolyLV = new G4LogicalVolulme(gemPoly, C2H4, "gemPolyLV", 0, 0, 0);
    ...
    G4AssemblyVolume* TripleGEM = new G4AssemblyVolume();
    G4RotaionMatrix Ra;  //
    G4ThreeVector Ta;    // for components
    G4RotationMatrix Rm; // for Assembly inside Hodoscope

    Ra.RotateX(0.5 * M_PI);
    Ra.RotateZ(0.5 * M_PI);
    TripleGEM->AddPlacedVolume(geomCuTopLV, G4Transform3D(Ra, Ta));
}
